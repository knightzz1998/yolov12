...
set_logging(name="CLFT", verbose=True)
logger = logging.getLogger("CLFT")
...
class CLFT(nn.Module):
    def __init__(self, in_dim, out_dim, in_feature, out_feature):
        ...
        # 打印输入输出维度
        logger.info(f"CLFT: in_dim:{in_dim}, out_dim:{out_dim}, in_feature:{in_feature}, out_feature:{out_feature}")
        self.attention = ConvAttention(in_dim, in_feature, out_feature)
        self.feedforward = FeedForward(in_dim, out_dim)

    def forward(self, x):
        # 打印输入维度
        logger.info(f"forward CLFT: input shape:{x.shape}")
        x = self.attention(x)
        out = self.feedforward(x)
        logger.info(f"forward CLFT: output shape:{out.shape}")
        return out
...
def test_custom():
    ...
    out_dim = 64  # Output channels
    in_feature = 128 * 128  # (height * width)
    out_feature = 128  # (height or width)

    # Instantiate the CLFT model
    clft = CLFT(in_dim, out_dim, in_feature, out_feature)

    # Create a sample input tensor with shape (batch_size, in_channels, height, width)
    # forward CLFT: input shape:torch.Size([1, 16, 128, 128])
    batch_size = 1
    height = 128
    width = 128
    #in_dim = 16
